...
/**
 * Tests for the fonts + examples store.
 *
...
describe('stores/resources', () => {
    it('loads fonts and examples in parallel', async () => {
        setApi(makeStubApi({
            '/typst/fonts': () => ({ fonts: [{ name: 'Inter-Regular.otf', kind: 'font', origin: 'skill', size: 609600, modified_at: 1700000000 }] }),
            '/typst/examples': () => ({ examples: [{ name: 'invoice.typ', kind: 'example', origin: 'skill', size: 1400, modified_at: 1700000000 }] }),
        }))
        ...
        expect(store.examples[0]?.name).toBe('invoice.typ')
